...
object Binarizer {
  ...
  case class Params(input: String = "", //输入数据,parquet格式
                    ...
                    inputCol: String = "feature", //输入列名
                    outputCol: String = "binarized_feature", //输出列名
                    threshold:Double=0.5,//阈值
                    resultCols: String = "", //输出结果保留的列,默认全部输出
                    appName: String = "Binarizer"
                   )
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("Binarizer") {
      head("Binarizer:.")
      opt[String]("input")
        .required()
        .text("输入数据")
  ...
  def run(p: Params): Unit = {
    ...
    val outputDF = new Binarizer()
      .setInputCol(p.inputCol)//feature
      .setOutputCol(p.outputCol)//binarized_feature
      .setThreshold(p.threshold)//0.5
      .transform(inputDF)
