...
/**
  ...
  */
object RandomForestClassifierTrain {
  /** 命令行参数 */
  case class Params(train_data: String = "", //训练数据路径
                    model_out: String = "",  //模型保存路径
                    appname: String = "RandomForestClassifier_Train",
                    num_classes:Int=2,//类别数
                    num_trees :Int= 3,//树的个数
                    feature_subset_strategy: String = "auto",//特征策略
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("RandomForestClassifier_Train") {
      head("RandomForestClassifier_Train: 随机森林分类训练.")
  ...
  def run(p:Params): Unit = {
    ...
    val model = RandomForest.trainClassifier(training, p.num_classes, categoricalFeaturesInfo,
