...
object LogisticRegressionPredict {
  ...
  case class Params(test_data: String = "", //测试数据路径
                    ...
                    appname: String = "LogisticRegression_Predict"
                   )
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("LogisticRegression_Predict") {
      head("LogisticRegression_Predict: 逻辑回归预测.")
  ...
  def run(p:Params): Unit = {
    ...
    val model = LogisticRegressionModel.load(sc, p.model_path) //加载模型
