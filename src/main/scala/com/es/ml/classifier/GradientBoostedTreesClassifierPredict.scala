...
/**
  ...
  * GradientBoostedTreesClassifier 预测
  */
object GradientBoostedTreesClassifierPredict {
  /** 命令行参数 */
  case class Params(test_data: String = "", //测试数据路径
                    model_path: String = "", //模型路径
                    predict_out: String = "", //预测结果保存路径
                    appname: String = "GradientBoostedTreesClassifier_Predict"
                   )
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("GradientBoostedTreesClassifier_Predict") {
      head("GradientBoostedTreesClassifier_Predict: 梯度提升树分类预测.")
