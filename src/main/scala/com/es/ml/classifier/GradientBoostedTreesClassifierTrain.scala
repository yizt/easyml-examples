...
/**
  ...
  * GradientBoostedTreesClassifier 训练
  */
object GradientBoostedTreesClassifierTrain {
  /** 命令行参数 */
  case class Params(train_data: String = "", //训练数据路径
                    model_out: String = "", //模型保存路径
                    appname: String = "GradientBoostedTreesClassifier_Train",
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("GradientBoostedTreesClassifier_Train") {
      head("GradientBoostedTreesClassifier_Train: 梯度提升树分类训练.")
  ...
  def run(p:Params): Unit = {
    ...
    val boostingStrategy = BoostingStrategy.defaultParams(p.default_params)
     ...
     boostingStrategy.treeStrategy.numClasses = p.num_classes
